...
import { getSession } from "@data-hygiene/core";
...
const RecordCard = ({ record, index = 0, mode, isReassignMode, isSelected, onToggleSelection }) => {
  ...
  const stopAndNavigate = (e) => {
    e.stopPropagation();
    setIsClicked(true);
    navigate(`/details/${record.ExecutionId}`, { state: { from: isEditable ? "active" : mode } });
  };
  ...
  return (
    ...
    >
      {isReassignMode && (
        <Box sx={{ pl: 1, pr: 1, flexShrink: 0, display: "flex", alignItems: "center" }}>
          <Checkbox
            checked={isSelected}
            onChange={onToggleSelection}
            sx={{ p: 0, color: "#ccc", "&.Mui-checked": { color: "#1b1b1b" } }}
          />
        </Box>
      )}
      {/* Execution ID */}
      ...
      {!showStagePanelInsteadOfStatus && record.Status && record.Status !== "N/A" && (
        <Box sx={{ display: "flex", gap: 0.5, alignItems: "center", pt: 0.5, flexShrink: 0 }}>
          <IconButton
            ...
            title={isEditable ? "Edit Record" : "View Details"}
            sx={{
