"""
Exhaustive POS/NEG Functional QA Report Generator
Includes ALL detailed interactive flows + extensive negative edge cases.
Formats steps as numbered lists and expected behavior for maximum readability.
"""
...
# Palette
C = {
    "navy": "FF17233A", "white": "FFFFFFFF", "gray_bg": "FFF1F5F9",
...
# (ID, Type, Area, Component, Steps, Expected, Status, Priority)
TESTS = [
    # ── SEARCH BAR ──────────────────────────────────────
    ...
    ("F-NAV-02", "Positive", "Navigation", "Card Click Routing", 
     ...
     "• The app dynamically navigates to `/:id`.\n• The detail page renders correctly for that specific Execution ID.", "Not Run", "Critical"),
    ("F-NAV-N1", "Negative", "Navigation", "Invalid ID URL", 
     "1. Open the browser address bar.\n2. Manually navigate to `/invalid_execution_999`.", 
     "• API returns a 404 response.\n• DetailsPage gracefully renders a 'Record not found' message instead of a blank white screen.", "Not Run", "High"),
...
ws = wb.active
ws.title = "Exhaustive Pos-Neg QA"
ws.sheet_view.showGridLines = False

# Header
ws.merge_cells("A1:H1")
header = ws.cell(row=1, column=1, value="COMPLETE EXHAUSTIVE QA: All Interactive Flows + Pos/Neg Testing")
...
for r, (tcid, ttype, area, comp, steps, expt, stat, pri) in enumerate(TESTS, 3):
    bg = C["white"] if r % 2 == 0 else C["gray_bg"]
    
    ws.cell(row=r, column=1, value=tcid).border = bdr()
...
OUTPUT_PATH = r"d:\DH\Data-Hygiene\QA_Exhaustive_Functional_Report_v2.xlsx"
wb.save(OUTPUT_PATH)
print(f"Report saved to: {OUTPUT_PATH} with {len(TESTS)} total test cases.")
