...
const ChooseOtherValueDropdown = ({
  ...
  const { options, loading, fetchOptions } = useUniqueValues(primaryField, sutType);
      ...
      if (json?.metadata_records && Array.isArray(json.metadata_records)) {
        ...
        if (sutType) {
          records = records.filter(
            (r) =>
              r.sutType?.toLowerCase() === sutType.toLowerCase() ||
              r.sut_type?.toLowerCase() === sutType.toLowerCase()
          );
        }
        if (sutType?.toLowerCase() === "cloud" && records.length > 0) {
          const getCpusVal = (r) => {
            const meta = r?.metadata || {};
            const cpuKey = Object.keys(meta).find((k) => k.toLowerCase() === "cpu(s)");
            if (!cpuKey) return 0;
            const val = Number(meta[cpuKey]);
            return isNaN(val) ? 0 : val;
          };
          const maxCpus = Math.max(...records.map((r) => getCpusVal(r)));
          records = records.filter((r) => getCpusVal(r) === maxCpus);
        }
      } else if (json?.data) {
